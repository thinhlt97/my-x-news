...
- Luyện nghe: nút `#makeListen*` → `makeListen(provider)` → `renderListen(data, chosen)`. Trình phát `makeSpeaker(turns)` đọc tuần tự từng lượt (nối bằng `onend` cho ổn iOS/PWA), 2 giọng tiếng Anh khác nhau cho 2 người, có ▶/⏸/🔁 + chỉnh tốc độ. Chấm điền từ bằng `normAns()` so với `answer`+`accept`. Từ vừa nghe cũng vào tab "Đã ôn" và tăng `quizCount` (như tạo đề).
